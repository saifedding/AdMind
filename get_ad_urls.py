...
def extract_ad_urls(data, ad_id):
    ...
    try:
        # Navigate to collation results (for collation details query)
        ad_cards = data.get('data', {}).get('ad_library_main', {}).get('collation_results', {}).get('ad_cards', [])
        
        # If not found, try search results (for foundation root query)
        if not ad_cards:
            edges = data.get('data', {}).get('ad_library_main', {}).get('search_results_connection', {}).get('edges', [])
            for edge in edges:
                collated_results = edge.get('node', {}).get('collated_results', [])
                ad_cards.extend(collated_results)
...
def main(ad_id):
    ...
    # First, try to get collation group ID by searching for the ad
    # This uses the AdLibraryFoundationRootQuery to find the collation_id
    search_variables = {
        "adType": "ALL",
        "activeStatus": "active",
    ...
    print("Searching for ad to get collation group ID...")
    ...
    if not collation_id:
        print(f"Could not find collation group ID for ad {ad_id}")
        return
    
    print(f"Found collation group ID: {collation_id}")
    print(f"Fetching detailed ad data...")
    
    # Now use the collation details query with the collation_id
    session.headers.update({
    ...
    for line in response_text.split('\n'):
        ...
        try:
            data_obj = json.loads(line)
            # Look for the object that contains ad_library_main with collation_results
            if 'data' in data_obj and 'ad_library_main' in data_obj.get('data', {}):
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python get_ad_urls.py <ad_id>")
        print("Example: python get_ad_urls.py 31103441552603633")
        sys.exit(1)
